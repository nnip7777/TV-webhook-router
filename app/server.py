...
function setRebuildBusy(busy){
  document.getElementById('rebuildBtn').disabled=busy;
  document.getElementById('rebuildTodayBtn').disabled=busy;
}
async function rebuildTodayAnalytics(){
  const status=document.getElementById('rebuildStatus');
  if(!window.confirm('Честно пересчитать только текущий день? Сначала будет восстановлен стартовый state на 00:00 MSK из всех более ранних fills, затем пересчитан текущий день.')) return;
  setRebuildBusy(true);
  status.innerHTML='Идёт rebuild today only…';
  try{
...
async function rebuildAnalytics(){
  const status=document.getElementById('rebuildStatus');
  if(!window.confirm('Пересобрать всю производную аналитику из сохранённых fills/executions? Это честно пересчитает round-trips, daily stats и open lots по всей истории.')) return;
  setRebuildBusy(true);
  status.innerHTML='Идёт full rebuild…';
  try{
...
async function load(){
  ...
  try{
    ...
    document.getElementById('closeevents').innerHTML=renderTable([
      {key:'closed_at',label:'closed_at',render:v=>fmtTime(v)}, {key:'broker',label:'broker'}, {key:'symbol',label:'symbol'}, {key:'direction',label:'dir'},
      {key:'lots_closed',label:'lots',render:v=>fmtNum(v,0)}, {key:'opening_signals',label:'open signals',render:v=>fmtNum(v,0)},
      {key:'closed_qty_sum',label:'closed qty',render:(v,r)=>`${fmtNum(v,6)} ${esc(r.qtyUnit || '')}`},
      {key:'gross_pnl_sum',label:'gross',render:(v,r)=>`<span class='${Number(v)>=0?'ok':'bad'}'>${fmtNum(v,4)} ${esc(r.pnlUnit || '')}</span>`},
      {key:'commission_sum',label:'fee',render:(v,r)=>`${fmtNum(v,4)} ${esc(r.pnlUnit || '')}`},
      {key:'net_pnl_sum',label:'net',render:(v,r)=>`<span class='${Number(v)>=0?'ok':'bad'}'>${fmtNum(v,4)} ${esc(r.pnlUnit || '')}</span>`},
      {key:'closing_signal_id',label:'close_signal',render:v=>`<code>${esc(v||'')}</code>`}
    ], data.latestCloseEvents || []);
    ...
    document.getElementById('daily').innerHTML=renderTable([
      ...
      {key:'trades_count',label:'trades',render:v=>fmtNum(v,0)},
      {key:'gross_pnl_sum',label:'gross (quote)',render:v=>`<span class='${Number(v)>=0?'ok':'bad'}'>${fmtNum(v,4)}</span>`},
      {key:'commission_sum',label:'fee (quote)',render:v=>fmtNum(v,4)},
...
def _request_open_qty_unit(request: Dict[str, Any]) -> str:
    signal_mode = str(request.get('signalMode') or '').strip().lower()
    broker = str(request.get('broker') or '').strip().lower()
    ...
    if signal_mode != 'target-direction':
        return qty_kind or open_qty_kind or 'contracts'
    if open_qty_kind:
        return open_qty_kind
    if broker == 'bingx':
        return qty_kind or 'usdt'
...
def _request_qty_unit(request: Dict[str, Any]) -> str:
    ...
    qty_kind = str(request.get('qtyKind') or '').strip().lower()
    if signal_mode == 'target-direction':
        return _request_open_qty_unit(request)
    return qty_kind or 'contracts'
...
def _request_sizing_basis(request: Dict[str, Any]) -> str:
    signal_mode = str(request.get('signalMode') or '').strip().lower()
    if signal_mode == 'target-direction':
        open_unit = _request_open_qty_unit(request)
...
def materialize_route(payload: Dict[str, Any], route: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for destination in route.get('destinations', []):
        ...
        if (payload_target_mode or str(rendered.get('signalMode') or '').strip().lower() == 'target-direction') and not rendered.get('openQtyKind'):
            rendered['openQtyKind'] = 'usdt' if str(rendered.get('broker') or '').strip().lower() == 'bingx' else (rendered.get('qtyKind') or 'contracts')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/api/effectiveness-rebuild':
            if not _has_permission(current_user, 'canViewJournal'):
                return self._json(403, {'error': 'forbidden'})
            try:
                return self._json(200, rebuild_analytics())
            except Exception as e:
                return self._json(500, {'error': 'analytics_rebuild_failed', 'details': str(e)})

        if self.path == '/api/effectiveness-rebuild-today':
            if not _has_permission(current_user, 'canViewJournal'):
                return self._json(403, {'error': 'forbidden'})
            try:
                return self._json(200, rebuild_analytics_today())
            except Exception as e:
                return self._json(500, {'error': 'analytics_rebuild_today_failed', 'details': str(e)})
