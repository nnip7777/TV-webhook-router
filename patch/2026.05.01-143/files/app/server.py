...
<div class='panel'><div class='nav'><a href='/'>admin</a><a href='/settings'>settings</a><a href='/journal'>journal</a><a href='/logout'>logout</a></div><h2 style='margin-bottom:6px;'>Эффективность сделок</h2><div class='muted'>Страница читает только готовые данные из SQLite, без пересчёта истории на старте сервера и без rebuild аналитики при открытии.</div></div>
...
<div class='panel'><h3 style='margin-top:0;'>Последние fills</h3><div class='muted' style='margin-bottom:8px;'>`qty` здесь, это фактический размер fill в активе инструмента. Для mode `open: USDT` это не USDT-сумма, а уже купленное количество актива. Для ясности ниже отдельно показан sizing basis.</div><div id='fills'></div></div>
...
async function load(){
  ...
  try{
    ...
    document.getElementById('latest').innerHTML=`
      <div class='card'><div class='k'>Последний signal</div><div><code>${esc(latestSignal.signal_id || '')}</code></div><div>${esc(latestSignal.received_at || '')}</div><div>${esc(latestSignal.source_ticker || '')} · ${esc(latestSignal.side || '')} · ${esc(latestSignal.qty_text || '')}</div></div>
      <div class='card'><div class='k'>Последний execution</div><div><code>${esc(latestExecution.execution_id || '')}</code></div><div>${esc(latestExecution.received_at || '')}</div><div>${esc(latestExecution.broker || '')} · ${esc(latestExecution.symbol || '')} · ${esc(latestExecution.status || '')}</div><div class='muted'>${esc(latestExecution.error_text || '')}</div></div>`;
    document.getElementById('fills').innerHTML=renderTable([
      {key:'observed_at',label:'time'}, {key:'broker',label:'broker'}, {key:'symbol',label:'symbol'}, {key:'phaseLabel',label:'phase'}, {key:'side',label:'side'},
      {key:'qty',label:'fill qty (asset)',render:v=>fmtNum(v,6)}, {key:'notional',label:'notional',render:v=>fmtNum(v,4)},
      {key:'price',label:'price',render:v=>fmtNum(v,6)}, {key:'sizingBasis',label:'sizing basis'},
      {key:'commission',label:'fee',render:(v,r)=>`${fmtNum(v,6)} ${esc(r.commission_currency || '')}`},
      {key:'signal_id',label:'signal_id',render:v=>`<code>${esc(v||'')}</code>`}
    ], data.latestFills || []);
    document.getElementById('roundtrips').innerHTML=renderTable([
      {key:'closed_at',label:'closed_at'}, {key:'broker',label:'broker'}, {key:'symbol',label:'symbol'}, {key:'direction',label:'dir'},
      {key:'entry_qty',label:'qty',render:v=>fmtNum(v,6)}, {key:'gross_pnl',label:'gross',render:v=>fmtNum(v,4)},
      {key:'commission_total',label:'fee',render:v=>fmtNum(v,4)}, {key:'net_pnl',label:'net',render:v=>`<span class='${Number(v)>=0?'ok':'bad'}'>${fmtNum(v,4)}</span>`},
    ...
    document.getElementById('daily').innerHTML=renderTable([
      ...
      {key:'trades_count',label:'trades',render:v=>fmtNum(v,0)}, {key:'gross_pnl_sum',label:'gross',render:v=>fmtNum(v,4)},
      {key:'commission_sum',label:'fee',render:v=>fmtNum(v,4)}, {key:'net_pnl_sum',label:'net',render:v=>fmtNum(v,4)}
