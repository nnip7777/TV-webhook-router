        ...
        def _run() -> Dict[str, Any]:
            ...
            if is_non_crypto_index and current_position_mode is True:
                ...
                if opposite_qty > 0:
                    ...
                    flip_open_after_close = True
                    request_payload['nettingAction'] = 'close_then_open_flip'
                    request_payload['closePositionSide'] = close_position_side
                    request_payload['openPositionSide'] = requested_position_side
                else:
            ...
            effective_position_side = requested_position_side
            if flip_open_after_close and close_position_side:
                close_prepared = dict(prepared)
                close_prepared['quantity'] = _decimal_text(min(abs(float(prepared['quantity'])), opposite_qty), _safe_int(contract_meta.get('quantityPrecision')))
                ...
                request_payload['flipCloseOrder'] = {
                    'positionSide': close_position_side,
                    'quantity': close_prepared['quantity'],
                    'resultCode': close_result.get('code'),
                    'resultMsg': close_result.get('msg'),
                }
                close_msg = str((close_result or {}).get('msg') or '')
                if close_result.get('code') not in (None, 0, '0') and 'No position to close' in close_msg:
                    request_payload['flipCloseOrder']['ignored'] = True
                elif close_result.get('code') not in (None, 0, '0'):
                    result = close_result
                else:
                    _set_stage('place_limit_order_flip_open')
