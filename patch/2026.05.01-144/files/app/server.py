...
<div class='panel'><h3 style='margin-top:0;'>Последние fills</h3><div class='muted' style='margin-bottom:8px;'>`fill qty` это фактический размер исполнения в базовом активе. `request size` и `sizing basis` показывают, чем был задан размер сигнала, например `10 usdt` для open и `0.35 contracts` для close. Комиссия концептуально считается от notional (`qty × price`), но источником истины остаётся фактическое поле `commission` из API биржи.</div><div id='fills'></div></div>
