    ...
    try:
        def _run() -> Dict[str, Any]:
            ...
            if risk_pct is not None and risk_pct > 0:
                ...
                qty_float = float(prepared['quantity'])
                allowed_loss = max(0.0, equity * (risk_pct / 100.0))
                notional = abs(mark_price * qty_float)
                target_margin = min(notional, allowed_loss) if allowed_loss > 0 else 0.0
                raw_leverage = (notional / target_margin) if target_margin > 0 else 1.0
                ...
                side_for_leverage = 'LONG' if side == 'buy' else 'SHORT'
                margin_ops['setMarginType'] = client.set_margin_type(prepared['symbol'], 'ISOLATED')
                margin_ops['setLeverage'] = client.set_leverage(prepared['symbol'], side_for_leverage, leverage)
            ...
            if risk_pct is not None and risk_pct > 0:
                risk_control = request_payload.get('riskControl') or {}
                target_margin = float(risk_control.get('targetMargin') or 0)
                leverage = max(1.0, float(risk_control.get('leverage') or 1.0))
                current_margin = abs(float(prepared['price']) * float(prepared['quantity'])) / leverage
                add_margin_amount = max(0.0, target_margin - current_margin)
                position_side_for_margin = position_side
                if position_side_for_margin == 'BOTH':
                    position_side_for_margin = 'LONG' if side == 'buy' else 'SHORT'
                if add_margin_amount > 0.00000001:
                    margin_ops['addMargin'] = client.adjust_isolated_margin(prepared['symbol'], position_side_for_margin, add_margin_amount, direction_type=1)
