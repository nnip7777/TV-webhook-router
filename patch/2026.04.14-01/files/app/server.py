...
def _render_journal_page(week: str = '', broker: str = '', ticker: str = '', status: str = '', kind: str = '', page: int = 1, sort: str = 'desc') -> str:
    ...
    for item in items:
        ...
        rows.append(f"<tr><td>{ts}</td><td>{kind}</td><td>{ticker}</td><td>{side}</td><td>{qty}</td><td>{brokers}</td><td>{status}</td><td>{source_file}</td></tr>")
    body_rows = ''.join(rows) or "<tr><td colspan='8'>Журнал пока пуст</td></tr>"
...
<body>
  <div class='panel'>
    ...
    <table>
      <thead><tr><th>time</th><th>kind</th><th>ticker</th><th>side</th><th>qty</th><th>brokers</th><th>status</th><th>file</th></tr></thead>
...
def main():
    ...
    append_journal({
        ...
        'brokers': [],
        'status': 'ok',
        'details': f'listening on http://{SERVER_HOST}:{SERVER_PORT}/ pid={os.getpid()} version={build.get("version","unknown")} serverHash={build.get("serverHash","")}',
