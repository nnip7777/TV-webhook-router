...
class SmartOrderExecutor:
    ...
    async def execute_smart_order(self, symbol: str, exchange: str, 
        ...
        """Исполнить один ордер на один входящий сигнал."""
        print(f"\n{'='*60}")
        ...
        if use_limit:
            ...
            print(f"\n3. Лимитная цена: {order_price}")
        else:
            print("\n1. Рыночное исполнение, стакан для цены не читаем")
        ...
        try:
            if self.broker == "alor":
                ...
                result = await self.client.place_order(
                    ...
                    quantity=order_payload['quantity'],
                    price=order_payload['price'],
                    order_type="ORDER_TYPE_LIMIT" if use_limit else "ORDER_TYPE_MARKET"
                )

            print(
                f"   ✅ Ордер: {order_payload['quantity']} @ {order_payload['price']}"
                if use_limit else f"   ✅ Market ордер: {order_payload['quantity']}"
            )
            return [{
                "status": "success",
                "order": order_payload,
