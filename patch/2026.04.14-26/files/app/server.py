...
def _render_admin_ui(config: Dict[str, Any], observed: Dict[str, Any]) -> str:
    ...
    for ticker in tickers:
        ...
        for broker_name, broker_cfg in brokers:
            ...
            symbol_value = html.escape(str(current.get('symbol') or best_candidate.get('symbol') or broker_cfg.get('symbolMap', {}).get(ticker, ticker)))
    ...
    def do_POST(self):
        ...
        try:
            ...
            for dest in destinations:
                ...
                err = str(dest.get('error') or result_obj.get('error') or '')
                dry_run = bool(dest.get('dryRun') or result_obj.get('dryRun'))
                if dry_run:
        ...
        destination_debug = []
        for dest in ((decision.get('executionResult') or {}).get('destinations') or []):
        ...
        http_status = 200 if decision['status'] in ('executed', 'dry_run', 'partial_error') else 500
        return self._json(http_status, decision)
