...
class SmartOrderExecutor:
    ...
    def split_order(self, quantity: int, total_value: float) -> List[Dict]:
        """Разбить крупную заявку на части только по количеству."""
    ...
    async def execute_smart_order(self, symbol: str, exchange: str, 
        ...
        orders_to_place = self.split_order(quantity, total_value)
        ...
        for i, order in enumerate(orders_to_place):
            try:
                order_price = price
                if use_limit:
                    live_orderbook = await self.client.get_orderbook(exchange, symbol)
                    live_analysis = self.analyze_orderbook(symbol, live_orderbook, side)
                    order_price = self.calculate_smart_price(live_analysis, side)
                    print(
                        f"   ↻ Заявка {i+1}: свежий стакан, bid={live_analysis.get('best_bid')} ask={live_analysis.get('best_ask')} -> price={order_price}"
                    )

                order_payload = {
                    "quantity": order['quantity'],
                    "price": _round_alor_price(symbol, order_price) if order_price is not None else None,
                }
                ...
                print(f"   ✅ Заявка {i+1}: {order_payload['quantity']} @ {order_payload['price']}")
                
            except Exception as e:
                failed_order = {
                    "quantity": order.get('quantity'),
                    "price": order_price if 'order_price' in locals() else None,
                }
                results.append({
                    "status": "error",
                    "order": failed_order,
