...
def _sync_broker_lookup_lists(config: Dict[str, Any], broker: str) -> Dict[str, Any]:
    ...
    return {
        'ok': True,
        'details': f"lookup synced: symbols={len(symbols)} venues={len(venues)} totalSymbols={len(broker_cfg.get('lookupSymbols', []))}",
    }
...
def _render_settings_page(saved: bool = False, error: str = '', message: str = '', tests: Dict[str, Dict[str, Any]] = None) -> str:
    ...
    section_fields = lambda section: ''.join(_render_env_input(field, env_values, root_value) for field in SETTINGS_FIELDS if field.get('section') == section)
...
def _result_error_text(result_obj: Any) -> str:
    if isinstance(result_obj, dict):
        return str(result_obj.get('error') or '')
    if isinstance(result_obj, list):
        errors = [str(item.get('error') or '') for item in result_obj if isinstance(item, dict) and item.get('error')]
...
def _render_journal_page(week: str = '', broker: str = '', ticker: str = '', status: str = '', kind: str = '', page: int = 1, sort: str = 'desc') -> str:
    ...
    for item in items:
        ...
        rows.append(
            f"<tr class='{' '.join(row_classes)}'><td>{ts}</td><td>{kind_label}</td><td>{ticker_label}</td><td>{side_label}</td><td>{qty_label}</td><td>{brokers_label}</td><td>{status_label}</td><td>{version}</td><td>{server_hash}</td><td class='details-cell'>{details}</td><td>{source_file}</td></tr>"
        )
...
<head>
  ...
  <style>
    ...
    .journal-row-server-start td {{ background: rgba(59, 130, 246, 0.10); border-top: 1px solid rgba(147, 197, 253, 0.30); border-bottom: 1px solid rgba(147, 197, 253, 0.16); }}
    .journal-row-server-start td:first-child {{ box-shadow: inset 3px 0 0 rgba(147, 197, 253, 0.50); }}
...
def _routing_details(route: Dict[str, Any]) -> str:
    ...
    for destination in route.get('destinations', []) or []:
        ...
        venue = destination.get('category') if broker == 'bybit' else destination.get('exchange')
...
def _current_atomic_qty(config: Dict[str, Any], ticker: str, instrument: Dict[str, Any], row_mapping: Dict[str, Dict[str, Any]], observed_entry: Dict[str, Any]) -> str:
    route_exists = False
    for route in config.get('routes', []):
    ...
    observed_qty = _observed_signal_qty(observed_entry)
    if observed_qty:
        return observed_qty

    if not route_exists:
        broker_sync_qty = _observed_broker_sync_qty(observed_entry)
        if broker_sync_qty:
            return broker_sync_qty
...
def _render_admin_ui(config: Dict[str, Any], observed: Dict[str, Any]) -> str:
    ...
    for broker_name, broker_cfg in brokers:
        venue_value = html.escape(str('linear' if broker_name == 'bybit' else broker_cfg.get('defaultDestination', {}).get('exchange', '')))
        ...
        manual_broker_cells.append(
            f'<td class="broker-cell broker-disabled" data-broker-cell="{html.escape(broker_name)}" data-broker-symbol="">'
            f"<div class='symbol-line'><label class='checkline-inline'><input type='checkbox' name='map|{manual_ticker_placeholder}|{html.escape(broker_name)}'></label>"
            f"<input list='{symbol_list_id}' autocomplete='off' class='mini-input symbol-input' type='text' name='symbol|{manual_ticker_placeholder}|{html.escape(broker_name)}' value='' placeholder='symbol'>"
            f"<input list='{venue_list_id}' autocomplete='off' class='mini-input venue-inline' type='text' name='venue|{manual_ticker_placeholder}|{html.escape(broker_name)}' value='{venue_value}' placeholder='{venue_placeholder}'>"
            f"<input autocomplete='off' class='mini-input position-inline qty-neutral' type='text' value='' placeholder='pos' readonly tabindex='-1'></div>"
            f"<div class='quick-line'><button type='button' class='quick-btn buy-btn' data-quick-order data-side='buy' data-broker='{html.escape(broker_name)}'>buy</button><input autocomplete='off' class='mini-input quick-qty qty-neutral' data-base-qty value='1' type='text' name='qty|{manual_ticker_placeholder}|{html.escape(broker_name)}' placeholder='qty'><button type='button' class='quick-btn sell-btn' data-quick-order data-side='sell' data-broker='{html.escape(broker_name)}'>sell</button><span class='cell-subhint'></span></div>"
            f"<input type='hidden' name='qtyMultiplier|{manual_ticker_placeholder}|{html.escape(broker_name)}' value=''>"
            '</td>'
        )

    manual_row = (
        '<tr class="mapping-row manual-row manual-row-disabled" data-manual-row="1" data-ticker="">'
        f"<td class='save-cell'><button type='submit' class='row-save row-save-inline' data-row-save disabled>Save</button></td>"
        f"<td class='ticker-cell'><div class='ticker-line'><label class='checkline-inline'><input type='checkbox' name='manualRowEnabled'></label><input autocomplete='off' class='mini-input symbol-input manual-ticker-input' type='text' name='manualTicker' value='' placeholder='new ticker'></div><div class='ticker-atomic-line'><span class='muted'>default qty</span><input autocomplete='off' class='mini-input atomic-qty-input' type='text' name='atomicQty|{manual_ticker_placeholder}' value='' placeholder='default qty'></div><div class='muted'>ручное правило, без входящего webhook</div></td>"
    ...
    for ticker in tickers:
        ...
        atomic_qty_value = _current_atomic_qty(config, ticker, instrument, row_mapping, obs)
        broker_cells = []
        for broker_name, broker_cfg in brokers:
            current = row_mapping.get(broker_name, {})
            ...
            venue_value = html.escape(str(current.get('venue') or best_candidate.get('venue') or ('linear' if broker_name == 'bybit' else broker_cfg.get('defaultDestination', {}).get('exchange', ''))))
            venue_placeholder = 'cat' if broker_name == 'bybit' else 'exch'
            margin_symbol = str(current.get('symbol') or broker_cfg.get('symbolMap', {}).get(ticker, ticker))
            order_qty_value = _current_order_qty(current, atomic_qty_value or '1')
            ...
            broker_cells.append(
                ...
                f"<div class='quick-line'><button type='button' class='quick-btn buy-btn' data-quick-order data-side='buy' data-broker='{html.escape(broker_name)}'>buy</button><input autocomplete='off' class='mini-input quick-qty {order_qty_class}' data-base-qty value='{html.escape(order_qty_value or atomic_qty_value or '1')}' type='text' name='qty|{html.escape(ticker)}|{html.escape(broker_name)}' placeholder='qty'><button type='button' class='quick-btn sell-btn' data-quick-order data-side='sell' data-broker='{html.escape(broker_name)}'>sell</button><span class='cell-subhint'>{html.escape(margin_hint)}</span></div>"
  ...
  <script>
    ...
    async function saveRow(row, btn) {{
      ...
      row.querySelectorAll('input[name]').forEach(function(el) {{
        ...
        if (isManualRow) {{
          name = name.replace('atomicQty|__manual__', `atomicQty|${{ticker}}`);
          name = name.replace('|__manual__|', `|${{ticker}}|`);
        }}
        if (el.type === 'checkbox') {{
          ...
          document.querySelectorAll(`[data-broker-cell="${{broker}}"]`).forEach(function(cell) {{
            ...
            if (sym.qty !== undefined) {{
              ...
              if (positionInput) {{
                ...
                positionInput.classList.remove('qty-neg', 'qty-pos', 'qty-neutral');
