...
def _current_atomic_qty(config: Dict[str, Any], ticker: str, instrument: Dict[str, Any], row_mapping: Dict[str, Dict[str, Any]], observed_entry: Dict[str, Any]) -> str:
    ...
    for broker_name in ('alor', 'finam', 'bybit', 'bingx', 'schwab'):
...
def _live_broker_metrics(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        balance_rows = balance.get('data') or []
        balance_row = balance_rows[0] if isinstance(balance_rows, list) and balance_rows else {}
        metrics['bingx']['summary'] = {
            'portfolio': _fmt_num(balance_row.get('equity')),
            'cash': _fmt_num(balance_row.get('availableMargin') or balance_row.get('balance')),
            'go': _fmt_num(balance_row.get('usedMargin') or balance_row.get('freezedMargin')),
            'currency': str(balance_row.get('asset') or 'USDT'),
