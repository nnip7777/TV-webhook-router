...
class SmartOrderExecutor:
    ...
    def analyze_orderbook(self, orderbook: Dict, side: str) -> Dict:
    ...
    def calculate_smart_price(self, analysis: Dict, side: str) -> float:
        """Рассчитать умную цену для лимитки"""
        ...
        # Если спред большой - ставим лимитку посередине
        if spread > 0:
    ...
    def split_order(self, quantity: int, price: float, total_value: float) -> List[Dict]:
        """Разбить крупную заявку на части"""
        if total_value < self.config['split_threshold']:
            return [{"quantity": quantity, "price": _round_alor_price(symbol, price)}]
        ...
        for i in range(parts):
            ...
            # Небольшой разброс цены для каждой части
            price_offset = price * (1 + (i - parts/2) * 0.001)  # ±0.1%
            orders.append({
                "quantity": qty,
                "price": _round_alor_price(symbol, price_offset)
            })
    ...
    async def execute_smart_order(self, symbol: str, exchange: str, 
        ...
        analysis = self.analyze_orderbook(orderbook, side)
        ...
        orders_to_place = self.split_order(quantity, price, total_value)
...
def _round_alor_price(symbol: str, price: float) -> float:
    """Round price to correct tick size for Alor symbol."""
    # Default tick size (can be overridden for specific symbols)
    tick_size = 0.01  # Default for most symbols
    
    # Special handling for futures
    if symbol.startswith("NG"):
        tick_size = 0.005  # NG futures typically use 0.005 tick size
    elif symbol.startswith("RTS"):
        tick_size = 0.5    # RTS index futures
    elif symbol.startswith("Si"):
        tick_size = 0.5    # Si index futures
