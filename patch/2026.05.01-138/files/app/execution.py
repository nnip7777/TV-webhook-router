...
def _bingx_build_fill_parts(snapshots: List[Dict[str, Any]], fallback_price: Any = None) -> List[Dict[str, Any]]:
    ...
    for snapshot in snapshots:
        ...
        current_commission = _safe_decimal(snapshot.get('commission'))
        ...
        def _run() -> Dict[str, Any]:
            ...
            def _run_limit_repost_loop(loop_prepared: Dict[str, Any], loop_position_side: str, loop_reduce_only: Any, stage_prefix: str = ''):
                ...
                for attempt_index in range(max_reposts + 1):
                    ...
                    attempt_entry['avgFillPrice'] = str(_bingx_order_avg_price(loop_final_order_row, current_prepared.get('price')))
                    attempt_entry['commissionTotal'] = str(sum((_safe_decimal(fill.get('commission')) for fill in attempt_entry.get('fills', []) if isinstance(fill, dict)), Decimal('0')))
                    attempt_entry['commissionCurrency'] = _bingx_order_commission_currency(loop_final_order_row)
