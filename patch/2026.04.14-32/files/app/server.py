...
def _render_journal_page(week: str = '', broker: str = '', ticker: str = '', status: str = '', kind: str = '', page: int = 1, sort: str = 'desc') -> str:
    ...
    for item in items:
        ...
        rows.append(
            f"<tr class='{' '.join(row_classes)}'><td>{ts}</td><td>{kind_label}</td><td>{ticker_label}</td><td>{side_label}</td><td>{qty_label}</td><td>{brokers_label}</td><td>{status_label}</td><td>{version}</td><td>{server_hash}</td><td>{details}</td><td>{source_file}</td></tr>"
        )
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                details = f"jobId={(job or {}).get('jobId')} | payload={_short_json(payload)} | error={e}"
                try:
                    with LOG_PATH.open('a') as f:
                        f.write(json.dumps({
                ...
                try:
                    append_journal({
                        'time': (job or {}).get('receivedAt') or _utcnow_iso(),
                        'kind': 'webhook',
                        'ticker': payload.get('sourceTicker'),
                        'side': payload.get('side'),
                        'qty': payload.get('qty'),
                        'brokers': [d.get('broker') for d in ((job or {}).get('route') or {}).get('destinations', []) if d.get('broker')],
...
def _enqueue_webhook_job(payload: Dict[str, Any], materialized: Dict[str, Any], default_execution: Dict[str, Any]) -> Dict[str, Any]:
    ...
    append_journal({
        ...
        'brokers': [d.get('broker') for d in materialized.get('destinations', []) if d.get('broker')],
        'status': 'accepted',
        'details': f"jobId={job['jobId']} | payload={_short_json(payload)} | route={_short_json(materialized, 280)} | queueDepth={decision['queueDepth']}",
    })
    return decision
