...
def _broker_connection_test(broker: str) -> Dict[str, Any]:
    ...
    if broker == 'bybit':
        try:
            ...
            return {'ok': api_info.get('retCode') == 0, 'text': str(api_info.get('retMsg') or api_info.get('retCode'))}
        except Exception as e:
            return {'ok': False, 'text': str(e)}
    if broker == 'finam':
        try:
            finam_result = finam_auth_check_sync()
            return {'ok': bool(finam_result.get('ok')), 'text': 'ok' if finam_result.get('ok') else str(finam_result.get('error') or 'error')}
        except Exception as e:
            return {'ok': False, 'text': str(e)}
    if broker == 'schwab':
        try:
            ...
            return {'ok': bool(schwab_result.get('ok')), 'text': 'ok' if schwab_result.get('ok') else str(schwab_result.get('error') or schwab_result.get('status_code') or 'error')}
        except Exception as e:
            return {'ok': False, 'text': str(e)}
    if broker == 'alor':
        try:
            if ALOR_CONFIG_PATH.exists():
                ...
                return {'ok': isinstance(positions, list), 'text': f'ok ({len(positions)} positions)'}
            return {'ok': False, 'text': 'config not found'}
        except Exception as e:
            return {'ok': False, 'text': str(e)}
    return {'ok': False, 'text': 'unknown broker'}
...
def _render_settings_page(saved: bool = False, error: str = '', message: str = '', tests: Dict[str, Dict[str, Any]] = None) -> str:
    ...
    def broker_header(name: str) -> str:
        return f"<div class='broker-sync-head'><div class='broker-sync-title'>{html.escape(name.title())}</div><form method='post' action='/settings/test-broker' style='display:inline'><input type='hidden' name='broker' value='{html.escape(name)}'><button type='submit'>sync</button></form><div class='broker-sync-status'>{broker_status(name)}</div></div>"
...
def _render_journal_page(week: str = '', broker: str = '', ticker: str = '', status: str = '', kind: str = '', page: int = 1, sort: str = 'desc') -> str:
    ...
    rows = []
    for item in items:
        ts = html.escape(str(item.get('time') or item.get('receivedAt') or ''))
