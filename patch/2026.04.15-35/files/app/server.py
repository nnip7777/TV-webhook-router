...
def _live_symbol_qty_text(broker_name: str, symbol: str) -> str:
    broker_payload = ((METRICS_CACHE.get('data') or {}).get(broker_name) or {}) if broker_name else {}
    symbol_payload = ((broker_payload.get('symbols') or {}).get(symbol) or {}) if symbol else {}
    qty = symbol_payload.get('qty')
    if qty in (None, ''):
        return ''
    return _fmt_qty_text(qty)
...
def _render_admin_ui(config: Dict[str, Any], observed: Dict[str, Any]) -> str:
    ...
    for ticker in tickers:
        ...
        for broker_name, broker_cfg in brokers:
            ...
            position_qty_value = _live_symbol_qty_text(broker_name, symbol_raw)
