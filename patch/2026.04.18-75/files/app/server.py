...
def _sync_broker_lookup_lists(config: Dict[str, Any], broker: str) -> Dict[str, Any]:
    ...
    for symbol in active_symbols:
        limits_entry = (broker_cfg.get('symbolLimits') or {}).get(symbol) or (broker_cfg.get('symbolLimits') or {}).get(str(symbol).replace('-', '').upper()) or {}
        if limits_entry:
            active_with_limits.append(symbol)
    save_config(config)
    return {
        'ok': True,
        'details': f"lookup synced: symbols={len(symbols)} venues={len(venues)} totalSymbols={len(broker_cfg.get('lookupSymbols', []))} active={len(active_symbols)} activeWithLimits={len(active_with_limits)}",
      ...
      if (syncBtn) {{
        ...
        try {{
          ...
          if (!res.ok) setFlashStatus(`ошибка sync: ${{broker}}`, 'error');
            ...
            setFlashStatus(`sync: ${{broker}} active ${{active}}, limits ${{withLimits}}`, 'ok');
          }}
