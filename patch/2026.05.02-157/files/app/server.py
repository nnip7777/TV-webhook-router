...
from analytics import analytics_overview, init_analytics_db, record_execution_analytics, rebuild_analytics
...
<div class='panel'><div style='display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;'><div id='status' class='muted'>Загрузка…</div><div style='display:flex;flex-direction:column;align-items:flex-end;gap:6px;'><button id='rebuildBtn' type='button' onclick='rebuildAnalytics()'>Rebuild analytics</button><div id='rebuildStatus' class='muted'>По кнопке полный rebuild производной аналитики из сохранённых fills/executions.</div></div></div><div id='meta' class='grid' style='margin-top:12px;'></div></div>
...
async function rebuildAnalytics(){
  ...
  if(!window.confirm('Пересобрать производную аналитику из сохранённых fills/executions? Это честно пересчитает round-trips, daily stats и open lots.')) return;
  btn.disabled=true;
  status.innerHTML='Идёт rebuild…';
  try{
    const res=await fetch('/api/effectiveness-rebuild',{method:'POST',headers:{'Accept':'application/json'}});
    const data=await res.json();
    if(!res.ok){throw new Error(data.error || ('HTTP '+res.status));}
    status.innerHTML=`<span class='ok'>Rebuild completed</span> · fills: ${fmtNum(data.replayedFills||0,0)} · effects updated: ${fmtNum(data.recomputedEffects||0,0)} · round-trips: ${fmtNum(((data.counters||{}).round_trips)||0,0)}`;
    await load();
  }catch(err){
    status.innerHTML=`<span class='bad'>Rebuild failed</span> · ${esc(err.message || err)}`;
  }finally{
    btn.disabled=false;
  }
}
