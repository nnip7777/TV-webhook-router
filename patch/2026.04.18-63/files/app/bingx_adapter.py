...
class BingXBroker:
    ...
    def get_contract(self, symbol: str) -> Dict[str, Any]:
        result = self.get_contracts(symbol=symbol)
    ...
    def prepare_limit_order(self, symbol: str, side: str, qty: Any, price: Optional[Any] = None) -> Dict[str, Any]:
        ...
        if selected_price in (None, ''):
            book_result = self.get_book_ticker(symbol)
            ...
            selected_price = book.get('askPrice') if side_normalized == 'buy' else book.get('bidPrice')
            if selected_price in (None, '', '0'):
                raise RuntimeError(f'BingX best quote is unavailable for {symbol}')
