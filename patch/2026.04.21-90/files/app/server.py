...
def _sync_broker_lookup_lists(config: Dict[str, Any], broker: str) -> Dict[str, Any]:
    ...
    if broker == 'bybit':
        ...
        symbol_rows = symbols_v3.get('data') or []
        if isinstance(symbol_rows, dict):
            symbol_rows = [symbol_rows]
        contract_rows = contracts_v2.get('data') or []
        ...
        for item in symbol_rows:
            ...
            broker_open = str(item.get('brokerState', 'true')).lower() != 'false'
            if not api_open or not broker_open:
                filtered_out_closed += 1
            symbols.append(symbol)
            venues.append('swap')
            normalized_symbol = str(symbol).replace('-', '').upper()
            contract_item = contract_index.get(symbol) or contract_index.get(normalized_symbol) or {}
            limits[symbol] = {
