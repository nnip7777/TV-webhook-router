...
def _sync_broker_lookup_lists(config: Dict[str, Any], broker: str) -> Dict[str, Any]:
    ...
    if broker == 'bybit':
        ...
        for item in (data.get('data') or []):
            ...
            limits[symbol] = {
                'minQty': item.get('minQty') or item.get('minOrderQty') or item.get('tradeMinQuantity') or item.get('minTradeAmount'),
                'qtyStep': item.get('stepSize') or item.get('quantityStep') or item.get('qtyStep'),
                'priceStep': item.get('tickSize') or item.get('priceStep'),
                'raw': item,
            }
...
def _build_destination_for_broker(config: Dict[str, Any], broker_name: str, ticker: str, options: Dict[str, Any], instruments: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    symbol_limits = ((broker_cfg.get('symbolLimits') or {}).get(destination.get('symbol')) or (broker_cfg.get('symbolLimits') or {}).get(str(destination.get('symbol') or '').replace('-', '').upper()) or {})
    if symbol_limits:
...
def _render_admin_ui(config: Dict[str, Any], observed: Dict[str, Any]) -> str:
    ...
    for ticker in tickers:
        ...
        for broker_name, broker_cfg in brokers:
            ...
            limits = current.get('limits') or ((broker_cfg.get('symbolLimits') or {}).get(symbol_raw) or (broker_cfg.get('symbolLimits') or {}).get(str(symbol_raw).replace('-', '').upper()) or {})
            limit_parts = []
            if limits.get('minQty') not in (None, ''):
                limit_parts.append(f"min {limits.get('minQty')}")
            if limits.get('qtyStep') not in (None, ''):
                limit_parts.append(f"step {limits.get('qtyStep')}")
            if limits.get('priceStep') not in (None, ''):
                limit_parts.append(f"tick {limits.get('priceStep')}")
            if limit_parts:
...
<head>
  ...
  <style>
    ...
    .book-modal-card {{ width:min(760px, calc(100vw - 32px)); max-height: calc(100vh - 32px); overflow:auto; background:#111827; border:1px solid #374151; border-radius:12px; padding:14px; box-shadow: 0 20px 60px rgba(0,0,0,0.45); }}
    ...
    document.addEventListener('click', async function(e) {{
      ...
      if (syncBtn) {{
        ...
        try {{
          ...
          else setFlashStatus(`sync: ${{broker}} ok`, 'ok');
...
<body>
  ...
  <div class='book-modal' id='book-modal'><div class='book-modal-card'><div class='book-modal-head'><strong id='book-modal-title'>book</strong><button type='button' class='quick-btn' onclick="document.getElementById('book-modal').classList.remove('open')">close</button></div><pre class='book-pre' id='book-modal-pre'></pre></div></div>
    ...
    def do_POST(self):
        ...
        if self.path == '/admin/book-view':
            ...
            try:
                ...
                if broker == 'bybit':
                    client = BybitBroker(testnet=False)
                    venue = form.get('venue', ['linear'])[0].strip() or 'linear'
                    return self._json(200, {
                        'broker': broker,
                        'symbol': symbol,
                        'venue': venue,
                        'book': client._request('GET', '/v5/market/orderbook', params={'category': venue, 'symbol': symbol, 'limit': 25}),
                    })
            except Exception as e:
