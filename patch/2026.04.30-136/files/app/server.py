...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route_path == '/effectiveness':
            return self._html(200, "<!doctype html><html lang='ru'><head><meta charset='utf-8'><title>Effectiveness</title><style>body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;margin:16px;background:#111827;color:#f3f4f6}.panel{background:#1f2937;padding:14px;border-radius:12px;max-width:920px;margin:0 auto}.nav a{color:#93c5fd;text-decoration:none;margin-right:12px}.muted{color:#9ca3af;font-size:12px}</style></head><body><div class='panel'><div class='nav'><a href='/'>admin</a><a href='/settings'>settings</a><a href='/journal'>journal</a><a href='/logout'>logout</a></div><h2 style='margin-bottom:8px;'>Эффективность сделок</h2><div class='muted'>Вкладка временно отключена в patch 136, чтобы убрать регрессию производительности. Я верну ее отдельным безопасным патчем после точечной переработки без влияния на admin/startup.</div></div></body></html>")
