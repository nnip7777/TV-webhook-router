...
def _render_admin_ui(config: Dict[str, Any], observed: Dict[str, Any]) -> str:
    ...
    for broker_name, broker_cfg in brokers:
        ...
        manual_broker_cells.append(
            ...
            f"<div class='symbol-line'><label class='checkline-inline'><input type='checkbox' name='map|{manual_ticker_placeholder}|{html.escape(broker_name)}' disabled></label>"
            f"<input list='{symbol_list_id}' autocomplete='off' class='mini-input symbol-input' type='text' name='symbol|{manual_ticker_placeholder}|{html.escape(broker_name)}' value='' placeholder='symbol' disabled>"
            f"<input list='{venue_list_id}' autocomplete='off' class='mini-input venue-inline' type='text' name='venue|{manual_ticker_placeholder}|{html.escape(broker_name)}' value='{venue_value}' placeholder='{venue_placeholder}' disabled>"
            f"<input autocomplete='off' class='mini-input position-inline qty-neutral' type='text' value='' placeholder='pos' readonly tabindex='-1' disabled></div>"
            f"<div class='quick-line'><button type='button' class='quick-btn buy-btn' data-quick-order data-side='buy' data-broker='{html.escape(broker_name)}' disabled>buy</button><input autocomplete='off' class='mini-input quick-qty qty-neutral' data-base-qty value='1' type='text' name='qty|{manual_ticker_placeholder}|{html.escape(broker_name)}' placeholder='qty' disabled><button type='button' class='quick-btn sell-btn' data-quick-order data-side='sell' data-broker='{html.escape(broker_name)}' disabled>sell</button><span class='cell-subhint'>сначала добавь тикер</span></div>"
    ...
    document.addEventListener('click', async function(e) {{
      ...
      if (quickBtn) {{
        ...
        try {{
          ...
          const brokerOk = String((data || {{}}).status || '') === 'ok';
    ...
    def do_POST(self):
        ...
        if self.path == '/admin/quick-order':
            ...
            if broker == 'bybit':
                ...
                if broker_ok:
                    broker_error = f"status_code={status_code}" if status_code else ''
            else:
