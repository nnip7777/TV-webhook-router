...
- Opposite positions are now closed first by actual current position size, then the target side is opened/added using the configured route `qty`.
- Same-side target-direction signals now add to the existing position instead of flattening or replacing it.
- Finam target-direction flows are now routed through `external/smart_order_executor.py` so Finam and Alor share the same stepwise limit-order behavior.

- Added a new `Последние close events` summary section to Effectiveness.
- It groups multiple lot-level `round_trips` that share the same closing signal into one user-readable close event row, with summed qty/gross/fee/net and the count of lots/signals closed.
- This makes flip/close actions like NASDAQ or SPY much easier to inspect before diving into the per-lot round-trip table.

## 2026.05.02-158
- Added a second Effectiveness rebuild mode: `Rebuild today only`.
- This mode first reconstructs the honest starting state at `00:00 MSK` by replaying all earlier fills into `open_lots`, then clears prior derived day outputs and replays only today's fills to rebuild today's `round_trips` and `daily_trade_stats`.
- Kept the full rebuild button as well, so the page now supports both whole-history recovery and a faster current-day-only recalculation with a correct carried-in position state.

## 2026.05.02-157
- Added an Effectiveness page button plus API endpoint to rebuild derived analytics from the stored raw source data on demand.
- The rebuild now replays saved `fills` joined with `executions.request_json`, recomputes `position_effect` using the current semantics, clears derived state (`open_lots`, `round_trips`, `round_trip_fills`, `daily_trade_stats`), and rebuilds it honestly from source records.
- This gives a user-triggered recovery path when semantics change or a live day looks suspicious, without requiring a manual SQLite wipe.

## 2026.05.02-156
- Refined the new fill semantics further for manual reduce flows: `reduce long` / `reduce short` now first consume the existing position, but if fill qty exceeds what was open they flip the remainder into the opposite side instead of dropping it.
- This matches the intended trading meaning of reduce actions with oversize qty: close what exists first, then open the residual as the opposite direction.
- Kept explicit semantic labels (`add long`, `add short`, `reduce long`, `reduce short`) so the Effectiveness page stays debuggable while analytics matching follows real flip behavior.

## 2026.05.02-155
- Refined Effectiveness fill semantics so target-direction adds are no longer treated as ambiguous `primary -> open_or_close` fills.
- Added explicit semantic labels for fills: `add long`, `add short`, `reduce long`, and `reduce short`, alongside the existing `open` and `close` states.
- `add long` / `add short` are now handled as open-only in analytics matching, while `reduce long` / `reduce short` are handled as close-only, preventing cases like SPY where a same-side add could be miscounted as closing an opposite lot.

## 2026.05.02-153
- Added sign-based color highlighting to the Effectiveness page daily aggregates table.
- In `Готовые дневные агрегаты`, both `gross (quote)` and `net (quote)` now render green for positive values and red for negative values, matching the round-trips visual treatment.
- `fee (quote)` remains neutral to avoid implying that higher fee values are good.

## 2026.05.01-152
- Fixed a core analytics lot-book bug where explicit `close` fills could leave residual quantity behind as a brand new open lot when analytics state was desynced.
- `target-close*` fills now only consume opposite open lots and no longer open fresh lots on leftover quantity, while explicit `target-open` fills skip opposite matching and open directly.
- This addresses the SPY-style failure mode where fills existed but the resulting `round-trips` became inconsistent or disappeared because the local lot state had been polluted.

## 2026.05.01-151
- Fixed a remaining journal mismatch where BingX `exec=` / `webhook-destination` request hints could still show `requestSize=... contracts` and `open=contracts` even for target-direction opens that are actually sized in USDT.
- The execution request payloads now carry explicit broker context, and journal unit inference now prefers `openQtyKind` when present.
- Also fixed Bybit target-direction to stop forcing `positionIdx` in one-way mode, which caused `retCode=10001 position idx not match position mode` on symbols like XAUTUSDT.

## 2026.05.01-150
- Fixed journal sizing hints for non-BingX `target-direction` routes so they no longer inherit BingX-style `open=usdt` labeling by mistake.
- Bybit target-direction entries like XAUTUSDT now show `requestSize` and `sizingBasis` in `contracts` when that is the real execution basis, instead of misleading `usdt` text.
- This cleans up the accepted/summary journal rows without changing the actual execution logic.

## 2026.05.01-149
- Hardened the Bybit adapter against `retCode=10002` timestamp drift errors.
- When Bybit returns a server-time mismatch, the router now re-syncs its local time offset from the API response or `/v5/market/time` and automatically retries the request once.
- This specifically addresses intermittent `invalid request, please check your server timestamp or recv_window param` failures like the XAUTUSDT target-direction error.

## 2026.05.01-148
- Normalized Effectiveness timestamps to the user's local timezone, MSK (`Europe/Moscow`), instead of mixing raw `Z` and `+08:00` strings from different sources.
- Latest signal/execution cards, fills time column, and round-trip close times now render in a single `DD-MM-YYYY HH:MM:SS MSK` format.
- Added an explicit note on the page that all times there are shown in MSK.

## 2026.05.01-147
- Added a BingX fee fallback via `/openApi/swap/v2/user/income` with `incomeType=TRADING_FEE` when `allFillOrders` and `allOrders` still return zero commission.
- When income rows are found for the executed order window, the router now allocates the factual fee amount across fills instead of leaving `fee = 0`.
- Polished the fills explanation text so it refers to factual BingX fee/income API data rather than only the `commission` field.

## 2026.05.01-146
- Simplified the Effectiveness `fill qty` unit for BingX swap fills to `cts` instead of repeating the full instrument/base name in every row.
- Updated the fills help text to explain that `cts` is used there intentionally to avoid duplicating the symbol column.

## 2026.05.01-145
- Extended journal rows and execution summaries with explicit quantity units and sizing hints.
- Webhook and quick-order journal entries now carry payload qty units in the main `qty` column when available.
- Destination/summary journal details now include `requestSize`, `sizingBasis`, `fillQtyUnit`, and `quoteUnit`, and target-direction close traces label contract-based quantities explicitly.

## 2026.05.01-144
- Added explicit unit display across the Effectiveness fills and round-trips sections.
- Recent fills now show request size, sizing basis, fill quantity units, price units, notional units, and fee units, plus a short note that fee conceptually comes from notional (`qty × price`) while the exchange API remains the source of truth.
- Round-trip PnL and fee rows now display quote-currency units explicitly instead of bare numbers.

## 2026.05.01-143
- Clarified the Effectiveness fills table so it no longer mixes sizing basis with actual filled asset quantity.
- Added explicit `phase` and `sizing basis` fields to recent fill rows, and relabeled `qty` as actual filled asset quantity while showing notional separately.

## 2026.05.01-142
- Fixed BingX `target-direction` sequencing when an opposite hedge leg already exists: the router now performs an explicit close-sized initial close pass before any target open logic, instead of accidentally starting with an open-sized order.
- Tagged the initial opposite-leg close attempts separately (`target-close-initial`) so analytics and journal traces reflect the real close-then-open order flow.
- Updated target-direction open handling to carry the final effective position side from the actual open pass.
