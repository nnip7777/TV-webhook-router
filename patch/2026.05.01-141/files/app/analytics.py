    ...
    with _DB_LOCK:
        with _connect() as conn:
            ...
            latest_fills = [dict(row) for row in conn.execute(
                '''
                SELECT fill_id, observed_at, broker, symbol, venue, side, qty, price, notional, commission, commission_currency, order_local_id, execution_id, signal_id
                FROM fills
                ORDER BY observed_at DESC, fill_id DESC
                LIMIT ?
                ''',
