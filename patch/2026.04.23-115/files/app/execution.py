...
def _bingx_position_id(position: Dict[str, Any]) -> str:
...
async def _execute_bingx(payload: Dict[str, Any], destination: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        def _run() -> Dict[str, Any]:
            ...
            if current_position_mode is True and close_position_side in ('LONG', 'SHORT'):
                opposite_position = _bingx_extract_position(positions_before_for_netting, prepared['symbol'], close_position_side)
                close_position_id = _bingx_position_id(opposite_position)
                request_payload['closePositionTarget'] = {
                    'positionSide': close_position_side,
                    'positionId': close_position_id,
                    'qty': _bingx_position_qty(opposite_position),
                }
                if not close_position_id:
                    raise RuntimeError(f'BingX hedge close target missing positionId for {prepared["symbol"]} {close_position_side}')
                _set_stage('close_position')
                result = client.close_position(close_position_id)
                effective_position_side = close_position_side
            else:
