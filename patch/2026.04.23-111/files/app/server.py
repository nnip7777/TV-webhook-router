...
WEBHOOK_DEDUP_LOCK = threading.Lock()
WEBHOOK_RECENT_KEYS: Dict[str, float] = {}
WEBHOOK_DEDUP_WINDOW_SECONDS = 2.0
...
def _enqueue_webhook_job(payload: Dict[str, Any], materialized: Dict[str, Any], default_execution: Dict[str, Any]) -> Dict[str, Any]:
    ...
    destinations = (materialized or {}).get('destinations') or []
    dedup_parts = [
        str(payload.get('sourceTicker') or ''),
        str(payload.get('side') or ''),
        str(payload.get('qty') or ''),
    ]
    for destination in destinations:
        dedup_parts.append('|'.join([
            str(destination.get('broker') or ''),
            str(destination.get('symbol') or ''),
            str(destination.get('side', payload.get('side')) or ''),
            str(destination.get('qty') or payload.get('qty') or ''),
        ]))
    dedup_key = '||'.join(dedup_parts)
    now_ts = time.time()
    with WEBHOOK_DEDUP_LOCK:
        expired = [key for key, ts in WEBHOOK_RECENT_KEYS.items() if (now_ts - ts) > WEBHOOK_DEDUP_WINDOW_SECONDS]
        for key in expired:
            WEBHOOK_RECENT_KEYS.pop(key, None)
        prev_ts = WEBHOOK_RECENT_KEYS.get(dedup_key)
        if prev_ts is not None and (now_ts - prev_ts) <= WEBHOOK_DEDUP_WINDOW_SECONDS:
            decision = {
            ...
            _append_multi_destination_journal(
                ...
                payload,
                materialized,
                lambda destination: f"payload={_short_json(payload)} | route={_short_json(destination or materialized, 280)} | dedupWindow={WEBHOOK_DEDUP_WINDOW_SECONDS}s",
            )
            return decision
        WEBHOOK_RECENT_KEYS[dedup_key] = now_ts
