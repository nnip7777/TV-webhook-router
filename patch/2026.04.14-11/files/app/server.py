...
ENV_KEYS_ORDER = [
    'WEBHOOK_ROUTER_ADMIN_PASSWORD',
    'WEBHOOK_ROUTER_HOST',
    'WEBHOOK_ROUTER_PORT',
    'WEBHOOK_ROUTER_PUBLIC_BASE_URL',
    'WEBHOOK_ROUTER_ROOT',
    'WEBHOOK_ROUTER_CONFIG_PATH',
    'WEBHOOK_ROUTER_OBSERVED_SIGNALS_PATH',
    'WEBHOOK_ROUTER_INSTRUMENTS_PATH',
    'WEBHOOK_ROUTER_LOG_PATH',
    'WEBHOOK_ROUTER_JOURNAL_LOG_PATH',
    'WEBHOOK_ROUTER_JOURNAL_KEEP_WEEKS',
    'WEBHOOK_ROUTER_SMART_EXECUTOR_PATH',
    'WEBHOOK_ROUTER_BACKUP_DIR',
    'WEBHOOK_ROUTER_BACKUP_KEEP_COUNT',
    'BYBIT_API_KEY',
    'BYBIT_SECRET_KEY',
    'BYBIT_LIVE_BASE_URL',
    'BYBIT_TESTNET_BASE_URL',
    'BYBIT_BIND_INTERFACE',
    'ALOR_CONFIG_PATH',
    'ALOR_OAUTH_URL',
    'ALOR_API_BASE_URL',
    'FINAM_SECRET_PATH',
...
def _is_admin_authenticated(headers) -> bool:
    ...
    if not password:
        return True
    token = _parse_cookie(headers.get('Cookie', '')).get('wr_admin_session', '')
...
<body>
  <form class='panel' method='post' action='/login'>
    ...
    <input class='mini-input' type='password' name='password' placeholder='password' autofocus>
...
def _render_settings_page(saved: bool = False, error: str = '', message: str = '', tests: Dict[str, Dict[str, Any]] = None) -> str:
    env_values = _ordered_env_values()
    tests = tests or {}
    ...
    def env_input(key: str, label: str, secret: bool = False, value_override: str = None) -> str:
        value = env_values.get(key, '') if value_override is None else value_override
    ...
    bybit_secret = env_values.get('BYBIT_SECRET_KEY', '')
    ...
    backup_rows = ''.join(
        f"<div class='backup-row'><code>{html.escape(item['name'])}</code><span class='help'>{html.escape(', '.join(item.get('files', [])))}</span><form method='post' action='/settings/backup/restore' style='display:inline'><input type='hidden' name='name' value='{html.escape(item['name'])}'><button type='submit'>restore</button></form><form method='post' action='/settings/backup/delete' style='display:inline'><input type='hidden' name='name' value='{html.escape(item['name'])}'><button type='submit'>delete</button></form></div>"
        for item in backups
...
<body>
  <div class='panel'>
    ...
    {flash}
    <form method='post' action='/settings/save' onsubmit='try{{if(document.activeElement&&document.activeElement.blur){{document.activeElement.blur();}}var bybitField=document.querySelector("input[name=&quot;env|BYBIT_BIND_INTERFACE&quot;]");var bybitShadow=document.getElementById("bybit-bind-interface-shadow");if(bybitField&&bybitShadow){{bybitShadow.value=bybitField.value||"";}}}}catch(e){{}}'>
      <div class='grid'>
        <div class='section'>
          <h3>Admin / server</h3>
          {env_input('WEBHOOK_ROUTER_ADMIN_PASSWORD', 'Admin password', secret=True)}
          {env_input('WEBHOOK_ROUTER_HOST', 'Host')}
          {env_input('WEBHOOK_ROUTER_PORT', 'Port')}
          {env_input('WEBHOOK_ROUTER_PUBLIC_BASE_URL', 'Public base URL')}
          {env_input('WEBHOOK_ROUTER_BACKUP_DIR', 'Backup dir', value_override=_display_path(root_value, env_values.get('WEBHOOK_ROUTER_BACKUP_DIR', '')))}
          {env_input('WEBHOOK_ROUTER_BACKUP_KEEP_COUNT', 'Backup keep count')}
          {env_input('WEBHOOK_ROUTER_JOURNAL_KEEP_WEEKS', 'Journal keep weeks')}
        </div>
        <div class='section'>
          <h3>Paths</h3>
          {env_input('WEBHOOK_ROUTER_ROOT', 'Root')}
          {env_input('WEBHOOK_ROUTER_CONFIG_PATH', 'Routing config path', value_override=_display_path(root_value, env_values.get('WEBHOOK_ROUTER_CONFIG_PATH', '')))}
          {env_input('WEBHOOK_ROUTER_OBSERVED_SIGNALS_PATH', 'Observed signals path', value_override=_display_path(root_value, env_values.get('WEBHOOK_ROUTER_OBSERVED_SIGNALS_PATH', '')))}
          {env_input('WEBHOOK_ROUTER_INSTRUMENTS_PATH', 'Instruments path', value_override=_display_path(root_value, env_values.get('WEBHOOK_ROUTER_INSTRUMENTS_PATH', '')))}
          {env_input('WEBHOOK_ROUTER_LOG_PATH', 'Webhook log path', value_override=_display_path(root_value, env_values.get('WEBHOOK_ROUTER_LOG_PATH', '')))}
          {env_input('WEBHOOK_ROUTER_JOURNAL_LOG_PATH', 'Journal log path', value_override=_display_path(root_value, env_values.get('WEBHOOK_ROUTER_JOURNAL_LOG_PATH', '')))}
          {env_input('WEBHOOK_ROUTER_SMART_EXECUTOR_PATH', 'Legacy smart_order_executor path', value_override=_display_path(root_value, env_values.get('WEBHOOK_ROUTER_SMART_EXECUTOR_PATH', '')))}
        </div>
        <div class='section'>
          {broker_header('bybit')}
          {env_input('BYBIT_API_KEY', 'Bybit API key')}
          <label class='settings-field'><span>Bybit secret key</span><input class='mini-input' type='password' name='env|BYBIT_SECRET_KEY' value='{html.escape(bybit_secret)}'></label>
          {env_input('BYBIT_LIVE_BASE_URL', 'Bybit live base URL')}
          {env_input('BYBIT_TESTNET_BASE_URL', 'Bybit testnet base URL')}
          {env_input('BYBIT_BIND_INTERFACE', 'Bybit bind interface (optional, e.g. bybit-egress)')}
          <input type='hidden' id='bybit-bind-interface-shadow' name='bybit_bind_interface_force' value='{html.escape(env_values.get('BYBIT_BIND_INTERFACE', ''))}'>
        </div>
        <div class='section'>
          {broker_header('alor')}
          {env_input('ALOR_CONFIG_PATH', 'Alor config JSON path', value_override=_display_path(root_value, env_values.get('ALOR_CONFIG_PATH', '')))}
          {env_input('ALOR_OAUTH_URL', 'Alor OAuth URL')}
          {env_input('ALOR_API_BASE_URL', 'Alor API base URL')}
        ...
        <div class='section'>
          {broker_header('finam')}
          {env_input('FINAM_ACCOUNT_ID', 'Finam account id')}
          {env_input('FINAM_SECRET_PATH', 'Finam token file path', value_override=_display_path(root_value, env_values.get('FINAM_SECRET_PATH', '')))}
        ...
        <div class='section'>
          ...
          {env_input('SCHWAB_CONFIG_PATH', 'Schwab config JSON path', value_override=_display_path(root_value, env_values.get('SCHWAB_CONFIG_PATH', '')))}
...
def _render_journal_page(week: str = '', broker: str = '', ticker: str = '', status: str = '', kind: str = '', page: int = 1, sort: str = 'desc') -> str:
    ...
    for item in items:
        ...
        ticker = html.escape(str(item.get('ticker') or item.get('sourceTicker') or ''))
        side = html.escape(str(item.get('side') or ''))
        qty = html.escape(str(item.get('qty') or ''))
        brokers = html.escape(', '.join(item.get('brokers') or []))
        status = html.escape(str(item.get('status') or ''))
        version = html.escape(str(item.get('version') or ''))
        server_hash = html.escape(str(item.get('serverHash') or ''))
        details = html.escape(str(item.get('details') or item.get('changedKeys') or ''))
        source_file = html.escape(str(item.get('_file') or ''))
        rows.append(f"<tr><td>{ts}</td><td>{kind}</td><td>{ticker}</td><td>{side}</td><td>{qty}</td><td>{brokers}</td><td>{status}</td><td>{version}</td><td>{server_hash}</td><td>{details}</td><td>{source_file}</td></tr>")
    body_rows = ''.join(rows) or "<tr><td colspan='11'>Журнал пока пуст</td></tr>"
    ...
    broker_options = ''.join(f"<option value='{name}' {'selected' if broker == name else ''}>{name}</option>" for name in ['alor','bybit','finam','schwab'])
    status_options = ''.join(f"<option value='{name}' {'selected' if status == name else ''}>{name}</option>" for name in ['executed','execution_error','route_not_found','invalid_json','missing_fields','ok','error'])
    ...
    next_link = f"/journal?week={html.escape(week)}&broker={html.escape(broker)}&ticker={html.escape(ticker)}&status={html.escape(status)}&kind={html.escape(kind)}&sort={html.escape(sort)}&page={page+1}" if end < total else ''
...
<body>
  <div class='panel'>
    ...
    <table>
      <thead><tr><th>time</th><th>kind</th><th>ticker</th><th>side</th><th>qty</th><th>brokers</th><th>status</th><th>version</th><th>server hash</th><th>details</th><th>file</th></tr></thead>
...
def load_config():
    return load_json(CONFIG_PATH, {'version': 1, 'brokers': {}, 'defaultExecution': {}, 'routes': []})
    ...
    for route in config.get('routes', []):
        for ticker in _extract_tickers_from_route(route):
            ...
            for destination in route.get('destinations', []):
                ...
                if not broker:
                    continue
                venue_key = 'category' if broker == 'bybit' else 'exchange'
...
def _build_destination_for_broker(config: Dict[str, Any], broker_name: str, ticker: str, options: Dict[str, Any]) -> Dict[str, Any]:
    broker_cfg = config.get('brokers', {}).get(broker_name, {})
    destination = copy.deepcopy(broker_cfg.get('defaultDestination', {}))
    destination['broker'] = broker_name
    symbol_map = broker_cfg.get('symbolMap', {})
    destination['symbol'] = (options.get('symbol') or '').strip() or symbol_map.get(ticker, ticker)

    venue_key = 'category' if broker_name == 'bybit' else 'exchange'
...
def _quick_order_base_qty(instrument: Dict[str, Any], broker_name: str, current: Dict[str, Any]) -> str:
    ...
    if broker_name == 'bybit':
        ...
        if 'crypto' in group or symbol.endswith('USDT'):
            return '0.01'
        return '1'
...
def _current_atomic_qty(config: Dict[str, Any], ticker: str, instrument: Dict[str, Any], row_mapping: Dict[str, Dict[str, Any]]) -> str:
    for route in config.get('routes', []):
        if ticker in _extract_tickers_from_route(route):
            existing = _route_atomic_qty(route)
            if existing:
                return existing
            break

    for broker_name in ('alor', 'finam', 'bybit', 'schwab'):
        current = row_mapping.get(broker_name, {})
        if current:
            return _quick_order_base_qty(instrument, broker_name, current)

    group = str((instrument or {}).get('group', '')).lower()
    if 'crypto' in group or str(ticker).upper().endswith('USDT'):
        return '0.01'
    return '1'
...
def _live_broker_metrics(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        resp = schwab.client.get_account(account_hash)
...
def _refresh_metrics_background(config: Dict[str, Any]) -> None:
    try:
        METRICS_CACHE['data'] = _live_broker_metrics(config)
...
def _get_live_metrics_cached(config: Dict[str, Any], max_age_seconds: int = 15) -> Dict[str, Any]:
    ...
    return copy.deepcopy(METRICS_CACHE.get('data') or {})
...
def _build_ui_route(config: Dict[str, Any], ticker: str, broker_options: Dict[str, Dict[str, Any]], atomic_qty: str = '') -> Dict[str, Any]:
    enabled_brokers = [
    ...
    destinations = [
        _build_destination_for_broker(config, broker_name, ticker, broker_options.get(broker_name, {}))
...
def save_single_ticker_mapping(config: Dict[str, Any], ticker: str, broker_options: Dict[str, Dict[str, Any]], atomic_qty: str = '') -> Dict[str, Any]:
    ...
    new_route = _build_ui_route(config, ticker, broker_options, atomic_qty)
...
def _render_admin_ui(config: Dict[str, Any], observed: Dict[str, Any]) -> str:
    brokers = [(name, cfg) for name, cfg in config.get('brokers', {}).items() if cfg.get('enabled', False)]
    ...
    header_cells = ''.join(
        ...
        f"</div><div class='broker-summary-line'><span class='broker-summary broker-summary-left' data-broker-portfolio='{html.escape(name)}'>портфель —</span><span class='broker-summary broker-summary-right' data-broker-cash='{html.escape(name)}'>остаток —</span></div><div class='broker-summary-line'><span class='broker-summary broker-summary-right' data-broker-go='{html.escape(name)}'>ГО —</span></div></th>"
    ...
    for ticker in tickers:
        ...
        atomic_qty_value = _current_atomic_qty(config, ticker, instrument, row_mapping)
        broker_cells = []
        for broker_name, broker_cfg in brokers:
            current = row_mapping.get(broker_name, {})
            is_checked = 'checked' if current else ''
            symbol_value = html.escape(str(current.get('symbol') or broker_cfg.get('symbolMap', {}).get(ticker, ticker)))
            venue_value = html.escape(str(current.get('venue') or ('linear' if broker_name == 'bybit' else broker_cfg.get('defaultDestination', {}).get('exchange', ''))))
            venue_placeholder = 'cat' if broker_name == 'bybit' else 'exch'
            qty_value = current.get('qty', '')
            qty_value_str = '' if qty_value in (None, '') else str(qty_value)
            qty_class = _qty_badge_class(qty_value_str)
            ...
            broker_cells.append(
                ...
                f"<input autocomplete='off' class='mini-input qty-inline {qty_class}' type='text' name='qty|{html.escape(ticker)}|{html.escape(broker_name)}' value='{html.escape(qty_value_str)}' placeholder='qty'></div>"
                f"<div class='quick-line'><button type='button' class='quick-btn buy-btn' data-quick-order data-side='buy' data-broker='{html.escape(broker_name)}'>buy</button><input autocomplete='off' class='mini-input quick-qty' data-base-qty value='{html.escape(atomic_qty_value or '1')}' type='text' placeholder='qty'><button type='button' class='quick-btn sell-btn' data-quick-order data-side='sell' data-broker='{html.escape(broker_name)}'>sell</button><span class='cell-subhint'>{html.escape(margin_hint)}</span></div>"
        ...
        rows.append(
            ...
            f"<td class='ticker-cell'><input type='hidden' name='ticker' value='{html.escape(ticker)}'><div class='ticker-line'><code>{html.escape(ticker)}</code></div><div class='ticker-atomic-line'><span class='muted'>default qty</span><input autocomplete='off' class='mini-input atomic-qty-input' type='text' name='atomicQty|{html.escape(ticker)}' value='{html.escape(atomic_qty_value)}' placeholder='default qty'></div><div class='muted'>{html.escape(meta_text)}</div><div class='muted'>{html.escape(instrument_group)}</div></td>"            f"{''.join(broker_cells)}"
            '</tr>'
        )
...
<head>
  ...
  <style>
    ...
    .cell-subhint {{ font-size: 11px; color:#aab4c0; margin-left: 6px; white-space: nowrap; }}
    .quick-line {{ display:flex; align-items:center; gap:4px; margin-top:4px; }}
    .quick-btn {{ padding: 3px 8px; border-radius: 6px; font-size: 11px; line-height: 1.1; }}
    .buy-btn {{ background: #14532d; color: #dcfce7; }}
    .sell-btn {{ background: #7f1d1d; color: #fee2e2; }}
    .quick-qty {{ width: 56px; min-width: 56px; text-align: center; margin: 0; }}
  ...
  <script>
    function updateQtyColors() {{
      document.querySelectorAll('input[name^="qty|"]').forEach(function(input) {{
        input.classList.remove('qty-neg', 'qty-pos', 'qty-neutral');
    ...
    async function saveRow(row, btn) {{
      ...
      if (tickerInput) formData.append('ticker', tickerInput.value);
      ...
      try {{
        const res = await fetch(form.getAttribute('action') || '/admin/save-mappings', {{
          method: 'POST',
    ...
    document.addEventListener('click', async function(e) {{
      ...
      if (quickBtn) {{
        ...
        const atomicInput = row.querySelector('.atomic-qty-input');
        const qty = (atomicInput || cell.querySelector('.quick-qty') || {{value:''}}).value;
        if (!qty) return;
        ...
        try {{
          ...
          const brokerOk = broker === 'bybit'
            ? (Number(brokerResult.retCode) === 0)
            : !brokerResult.error;
          const brokerErrorText = broker === 'bybit'
            ? (brokerResult.retMsg || brokerResult.raw || brokerResult.retCode || '')
            : (brokerResult.error || brokerResult.details || '');
          if (!res.ok || data.error) {{
        ...
        Object.entries(data || {{}}).forEach(function(entry) {{
          ...
          document.querySelectorAll(`[data-broker-cell="${{broker}}"]`).forEach(function(cell) {{
            ...
            if (sym.qty !== undefined) {{
              const qtyInput = cell.querySelector('input[name^="qty|"]');
              if (qtyInput && document.activeElement !== qtyInput) {{
                qtyInput.value = String(sym.qty);
                qtyInput.classList.remove('qty-neg', 'qty-pos', 'qty-neutral');
                const v = parseFloat(qtyInput.value);
                if (isNaN(v)) qtyInput.classList.add('qty-neutral');
                else if (v < 0) qtyInput.classList.add('qty-neg');
                else if (v > 0) qtyInput.classList.add('qty-pos');
                else qtyInput.classList.add('qty-neutral');
              }}
              ...
              if (quickInput && document.activeElement !== quickInput) {{
                const baseQty = (atomicInput && atomicInput.value) || quickInput.getAttribute('data-base-qty') || quickInput.value || '1';
  ...
  <form method='post' action='/admin/save-mappings' id='mapping-form' autocomplete='off' onsubmit='return false;'>
    ...
    <div class='panel'>
      <div class='toolbar'>
        ...
        <div style='display:flex; gap:10px; align-items:center;'><a href='/settings' style='color:#93c5fd;text-decoration:none;'>Settings</a><a href='/journal' style='color:#93c5fd;text-decoration:none;'>Journal</a><button type='button' disabled>Save all позже</button></div>
      </div>
      {''.join(lookup_lists)}
      <table>
        <thead>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route_path == '/logout':
            token = _parse_cookie(self.headers.get('Cookie', '')).get('wr_admin_session', '')
            if token:
                ADMIN_SESSIONS.pop(token, None)
            return self._redirect('/login', 'wr_admin_session=; Path=/; Max-Age=0; HttpOnly; SameSite=Lax')
        if self._require_admin():
            return
        if route_path == '/':
            config = load_config()
            observed = load_observed_signals()
            return self._html(200, _render_admin_ui(config, observed))
        if route_path == '/healthz':
            return self._json(200, {'status': 'ok', 'build': _build_summary()})
        if route_path == '/settings':
            ...
            return self._html(200, _render_settings_page(saved=('saved=1' in self.path), message=message))
        ...
        if route_path == '/api/broker-metrics':
            config = load_config()
            return self._json(200, _get_live_metrics_cached(config))
    ...
    def do_POST(self):
        ...
        if self.path == '/login':
            ...
            if password != _admin_password():
                return self._html(401, _render_login_page('wrong password'))
            token = _create_admin_session()
            return self._redirect('/', f'wr_admin_session={token}; Path=/; Max-Age={ADMIN_SESSION_TTL_SECONDS}; HttpOnly; SameSite=Lax')
        ...
        if self.path == '/settings/save':
            ...
            for key in form.keys():
                if key.startswith('env|'):
                    ...
                    raw_value = form.get(key, [''])[0]
                    new_value = _resolve_path_from_root(root_input, raw_value) if env_key in path_keys else raw_value
                    if env_values.get(env_key) != new_value:
                        changed_keys.append(env_key)
                    env_values[env_key] = new_value
            forced_bybit_bind_interface = form.get('bybit_bind_interface_force', [None])[0]
            if forced_bybit_bind_interface is not None:
                forced_bybit_bind_interface = forced_bybit_bind_interface.strip()
                if env_values.get('BYBIT_BIND_INTERFACE', '') != forced_bybit_bind_interface:
                    changed_keys.append('BYBIT_BIND_INTERFACE')
                env_values['BYBIT_BIND_INTERFACE'] = forced_bybit_bind_interface
        ...
        if self.path == '/settings/test-broker':
            ...
            if broker and broker in tests:
                ...
                append_journal({
                    ...
                    'details': info.get('details') or info.get('text') or '',
                })
            return self._html(200, _render_settings_page(message=f'{broker} sync finished' if broker else 'test finished', tests=tests))
        ...
        if self.path == '/admin/save-mappings':
            ...
            atomic_by_ticker: Dict[str, str] = {}
            for key in form.keys():
                if '|' not in key:
                    continue
                parts = key.split('|')
                field = parts[0]
                if field == 'atomicQty' and len(parts) == 2:
                    atomic_by_ticker[parts[1]] = form.get(key, [''])[0]
                    continue
                if len(parts) != 3:
                    continue
            ...
            config = load_config()
            if target_ticker:
                config = save_single_ticker_mapping(config, target_ticker, selections.get(target_ticker, {}), atomic_by_ticker.get(target_ticker, ''))
                return self._json(200, {'status': 'ok', 'savedTicker': target_ticker, 'routes': len(config.get('routes', []))})

            config = save_mappings(config, selections, atomic_by_ticker)
        ...
        if self.path == '/admin/quick-order':
            ...
            if broker == 'bybit':
                destination['category'] = venue or destination.get('category')
            else:
                destination['exchange'] = venue or destination.get('exchange')
            ...
            dry_run = bool(first_dest.get('dryRun') or broker_result.get('dryRun'))
            broker_ok = True
            broker_error = ''
            if broker == 'bybit':
            ...
            journal_status = 'dry_run' if dry_run else ('ok' if broker_ok else 'execution_error')
            append_journal({
                'time': _utcnow_iso(),
                'kind': 'quick-order',
                'ticker': ticker,
        ...
        decision = {
            'receivedAt': _utcnow_iso(),
            'payload': payload,
            'execution': config.get('defaultExecution', {}),
            'route': materialized,
            'status': 'accepted'
        }
        ...
        append_journal({
            ...
            'side': payload.get('side'),
            'qty': payload.get('qty'),
            'brokers': [d.get('broker') for d in materialized.get('destinations', []) if d.get('broker')],
            'status': decision.get('status'),
        })

        return self._json(200 if decision['status'] == 'executed' else 500, decision)
...
def main():
    ...
    append_journal({
        ...
        'brokers': [],
        'status': 'ok',
        'details': f'listening on http://{SERVER_HOST}:{SERVER_PORT}/ pid={os.getpid()} builtAt={build.get("builtAt","")} fileCount={build.get("fileCount","")}',
