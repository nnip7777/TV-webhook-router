...
def _fill_request_unit(phase: str, request_json: str) -> str:
    phase_label = _fill_phase_label(phase)
    request: Dict[str, Any] = {}
    try:
        loaded = json.loads(request_json or '{}')
...
def _fill_request_size(requested_qty_text: str, phase: str, request_json: str) -> str:
    qty_text = str(requested_qty_text or '').strip()
    if not qty_text:
        return ''
    return f"{qty_text} {_fill_request_unit(phase, request_json)}"
    ...
    with _DB_LOCK:
        with _connect() as conn:
            ...
            for row in latest_fills:
                ...
                row['phaseLabel'] = _fill_phase_label(phase)
                row['sizingBasis'] = _fill_sizing_basis(phase, request_json)
                row['requestSize'] = _fill_request_size(str(row.get('requested_qty_text') or ''), phase, request_json)
                row['fillQtyUnit'] = base_unit
                row['priceUnit'] = quote_unit
                row['notionalUnit'] = quote_unit
                row['feeUnit'] = str(row.get('commission_currency') or quote_unit)
                row['feeBasis'] = f"{base_unit} qty × price = notional in {quote_unit}"
