...
EFFECTIVENESS_CACHE_PATH = ROOT / 'logs' / 'effectiveness-cache.json'
EFFECTIVENESS_CACHE_MAX_ROWS = 12000
...
def _extract_bingx_risk_details(destination: Dict[str, Any]) -> str:
    ...
    if str(request.get('signalMode') or '').lower() == 'target-direction':
        ...
        if close_still_open not in (None, ''):
            parts.append(f"closeStillOpenQty={close_still_open}")
        ...
        if isinstance(close_attempts, list) and close_attempts:
            ...
            for item in close_attempts[:8]:
                if not isinstance(item, dict):
                    continue
                rendered.append(
                    f"#{item.get('pass')}:pos={item.get('remainingPositionQty')},ord={item.get('closeOrderRemainingQty','')}"
                )
            if rendered:
...
def _render_journal_page(week: str = '', broker: str = '', ticker: str = '', status: str = '', kind: str = '', page: int = 1, sort: str = 'desc') -> str:
    ...
    for item in items:
        ...
        qty_label = html.escape(str(item.get('qty') or ''))
...
def _extract_trade_summary(entry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    trade_summary = entry.get('tradeSummary') if isinstance(entry.get('tradeSummary'), dict) else {}
    request_data: Dict[str, Any] = {}
    result_data: Dict[str, Any] = {}
    if isinstance(trade_summary.get('request'), dict):
        request_data.update(trade_summary.get('request') or {})
    if isinstance(trade_summary.get('result'), dict):
        result_data.update(trade_summary.get('result') or {})
    ...
    fee_rate = _to_float(trade_summary.get('feeRate'), 0.0005)
    if fee_rate <= 0:
        broker_name = str((entry.get('brokers') or [trade_summary.get('broker') or ''])[0] or '').lower()
        fee_rate = 0.0005 if broker_name == 'bingx' else 0.0006 if broker_name == 'bybit' else 0.0
    ...
    legs = [item for item in (trade_summary.get('legs') or []) if isinstance(item, dict)]
    if not legs:
        legacy_attempts = []
        if isinstance(request_data.get('orderAttempts'), list):
            legacy_attempts.extend(request_data.get('orderAttempts') or [])
        if isinstance(request_data.get('targetOpenAttempts'), list):
            legacy_attempts.extend([{**item, 'phase': item.get('phase') or 'target-open'} for item in (request_data.get('targetOpenAttempts') or []) if isinstance(item, dict)])
        for attempt in legacy_attempts:
            if not isinstance(attempt, dict):
                continue
            executed_qty = _to_float(attempt.get('executedQty'))
            placed_price = _to_float(attempt.get('placedPrice'))
            derived_notional = abs(executed_qty * placed_price)
            phase = str(attempt.get('phase') or 'primary')
            final_status = str(attempt.get('finalStatus') or '')
            if derived_notional <= 0 and executed_qty <= 0 and not final_status:
                continue
            legs.append({
                'phase': phase,
                'orderId': str(attempt.get('orderId') or ''),
                'executedQty': executed_qty,
                'placedPrice': placed_price,
                'notional': derived_notional,
                'feeRate': fee_rate,
                'estimatedFee': derived_notional * fee_rate if derived_notional > 0 and fee_rate > 0 else 0.0,
                'finalStatus': final_status,
                'remainingQty': _to_float(attempt.get('remainingQty')),
            })
    executed_notional = sum(abs(_to_float(item.get('notional'))) for item in legs)
    executed_fees = sum(abs(_to_float(item.get('estimatedFee'))) for item in legs)
    open_legs = [item for item in legs if str(item.get('phase') or 'primary') in ('primary', 'target-open')]
    close_legs = [item for item in legs if 'close' in str(item.get('phase') or '')]
    open_notional = sum(abs(_to_float(item.get('notional'))) for item in open_legs)
    close_notional = sum(abs(_to_float(item.get('notional'))) for item in close_legs)
    gross_edge = close_notional - open_notional
    net_edge = gross_edge - executed_fees
    return {
        'ticker': str(entry.get('ticker') or ''),
        'symbol': str(entry.get('symbol') or trade_summary.get('symbol') or ''),
        'broker': str((entry.get('brokers') or [trade_summary.get('broker') or ''])[0] or ''),
        'venue': str(entry.get('venue') or trade_summary.get('venue') or ''),
        'side': side,
        'sideSign': side_sign,
        'qty': qty,
        ...
        'executedFees': executed_fees,
        'openNotional': open_notional,
        'closeNotional': close_notional,
        'grossEdge': gross_edge,
        'netEdge': net_edge,
        'hasLifecycle': bool(legs),
    }


def _effectiveness_default_cache() -> Dict[str, Any]:
    return {
        'version': 1,
        'generatedAt': '',
        'lastScan': {},
...
def _load_effectiveness_cache() -> Dict[str, Any]:
    if not EFFECTIVENESS_CACHE_PATH.exists():
        return _effectiveness_default_cache()
    try:
        payload = json.loads(EFFECTIVENESS_CACHE_PATH.read_text())
        if isinstance(payload, dict):
            payload.setdefault('version', 1)
            payload.setdefault('generatedAt', '')
            payload.setdefault('lastScan', {})
...
def _save_effectiveness_cache(cache: Dict[str, Any]) -> None:
    save_json(EFFECTIVENESS_CACHE_PATH, cache)


def _effectiveness_row_key(entry: Dict[str, Any]) -> str:
    return '|'.join([
        ...
        str(entry.get('details') or '')[:240],
    ])


def _effectiveness_iter_new_rows(cache: Dict[str, Any]) -> List[Dict[str, Any]]:
    last_scan = cache.get('lastScan') or {}
    rows = []
    for path in _journal_files():
        if path.suffix == '.gz':
            continue
        last_line = int((last_scan.get(path.name) or {}).get('lineCount') or 0)
        try:
        ...
        if last_line < 0:
            last_line = 0
        for line in current_lines[last_line:]:
            try:
                row = json.loads(line)
                row['_file'] = path.name
                rows.append(row)
    ...
    cache['lastScan'] = last_scan
    return rows


def _effectiveness_merge_rows(existing: List[Dict[str, Any]], incoming: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    seen = {_effectiveness_row_key(item): item for item in existing if isinstance(item, dict)}
    for item in incoming:
        if not isinstance(item, dict):
            continue
        key = _effectiveness_row_key(item)
        seen[key] = item
    merged = list(seen.values())
    merged.sort(key=lambda item: str(item.get('time') or ''), reverse=True)
    return merged[:EFFECTIVENESS_CACHE_MAX_ROWS] if len(merged) > EFFECTIVENESS_CACHE_MAX_ROWS else merged


def _effectiveness_finalize(rows: List[Dict[str, Any]], week: str = '', broker: str = '', ticker: str = '') -> Dict[str, Any]:
    filtered_trade_items = []
    for item in rows:
        if str(item.get('kind') or '') not in ('webhook-destination', 'quick-order-destination'):
            continue
        if str(item.get('status') or '') not in ('placed', 'dry_run', 'execution_error'):
            continue
        if week and week not in str(item.get('_file') or ''):
            continue
        if broker and broker not in [str(x).lower() for x in (item.get('brokers') or [])]:
            continue
        if ticker and ticker.lower() not in str(item.get('ticker') or '').lower():
            continue
        filtered_trade_items.append(item)

    parsed_rows = [_extract_trade_summary(item) for item in filtered_trade_items]
    parsed_rows = [row for row in parsed_rows if row.get('ticker')]
    ...
    for row in parsed_rows:
        ...
        bucket['sumExecutedFees'] += _to_float(row.get('executedFees'))
        bucket['sumGrossEdge'] += _to_float(row.get('grossEdge'))
        bucket['sumNetEdge'] += _to_float(row.get('netEdge'))
    ...
    for payload in per_ticker.values():
        ...
        lifecycle_count = int(payload.get('lifecycleCount') or 0)
        ...
        avg_gross_edge = (payload['sumGrossEdge'] / lifecycle_count) if lifecycle_count else 0.0
        avg_net_edge = (payload['sumNetEdge'] / lifecycle_count) if lifecycle_count else 0.0
        avg_executed_fees = (payload['sumExecutedFees'] / lifecycle_count) if lifecycle_count else 0.0
        commission_drag_pct = ((payload['sumExecutedFees'] / abs(payload['sumGrossEdge'])) * 100.0) if abs(payload['sumGrossEdge']) > 0 else 0.0
        suggested_multiplier = 1.0
        if lifecycle_count and payload['sumNetEdge'] <= 0 and abs(payload['sumGrossEdge']) > 0:
            suggested_multiplier = min(25.0, max(1.0, payload['sumExecutedFees'] / abs(payload['sumGrossEdge'])))
        items.append({
            ...
            'avgGrossEdge': avg_gross_edge,
            'avgNetEdge': avg_net_edge,
            'avgExecutedFees': avg_executed_fees,
            'commissionDragPct': commission_drag_pct,
            'suggestedLotMultiplier2x': suggested_multiplier,
            'sumNotional': payload['sumNotional'],
            'sumFeeRoundTrip': payload['sumFeeRoundTrip'],
            'sumExecutedFees': payload['sumExecutedFees'],
            'sumGrossEdge': payload['sumGrossEdge'],
            'sumNetEdge': payload['sumNetEdge'],
            'maxFeeSharePct': payload['maxFeeSharePct'],
        })
    items.sort(key=lambda item: (item.get('sumNetEdge', 0.0), -item.get('sumFeeRoundTrip', 0.0), item.get('ticker', '')))
    ...
    totals = {
        ...
        'lifecycleCount': sum(int(item.get('lifecycleCount') or 0) for item in items),
        ...
        'sumExecutedFees': sum(_to_float(item.get('sumExecutedFees')) for item in items),
        'sumGrossEdge': sum(_to_float(item.get('sumGrossEdge')) for item in items),
        'sumNetEdge': sum(_to_float(item.get('sumNetEdge')) for item in items),
    }
    totals['avgFeeSharePct'] = ((totals['sumFeeRoundTrip'] / totals['sumNotional']) * 100.0) if totals['sumNotional'] > 0 else 0.0
    totals['commissionDragPct'] = ((totals['sumExecutedFees'] / abs(totals['sumGrossEdge'])) * 100.0) if abs(totals['sumGrossEdge']) > 0 else 0.0
    return {
        ...
        'totals': totals,
        'items': items,
        'rows': parsed_rows[:200],
    }


def _reset_effectiveness_cache() -> None:
    try:
        if EFFECTIVENESS_CACHE_PATH.exists():
            EFFECTIVENESS_CACHE_PATH.unlink()
    except Exception:
        pass


def _journal_effectiveness(limit: int = 4000, week: str = '', broker: str = '', ticker: str = '') -> Dict[str, Any]:
    cache = _load_effectiveness_cache()
    new_rows = _effectiveness_iter_new_rows(cache)
    if new_rows:
        merged_rows = _effectiveness_merge_rows(cache.get('rows') or [], new_rows)
        cache['rows'] = merged_rows
        cache['generatedAt'] = _utcnow_iso()
        cache['lastDeltaRows'] = len(new_rows)
        _save_effectiveness_cache(cache)
    report = _effectiveness_finalize(cache.get('rows') or [], week=week, broker=broker, ticker=ticker)
    report['cacheMeta'] = {
        'generatedAt': cache.get('generatedAt') or '',
        'deltaRows': int(cache.get('lastDeltaRows') or 0),
        'cachedRows': len(cache.get('rows') or []),
        'maxRows': EFFECTIVENESS_CACHE_MAX_ROWS,
        'trackedFiles': len(cache.get('lastScan') or {}),
    }
    return report
...
def _render_effectiveness_page(week: str = '', broker: str = '', ticker: str = '') -> str:
    ...
    rows = []
    for item in items:
        net_class = ' style="color:#86efac;"' if _to_float(item.get('avgNetEdge')) > 0 else ' style="color:#fca5a5;"' if _to_float(item.get('avgNetEdge')) < 0 else ''
        rows.append(
            f"<tr>"
            ...
            f"<td>{int(item.get('lifecycleCount') or 0)}</td>"
            f"<td>{int(item.get('errorCount') or 0)}</td>"
            f"<td>{html.escape(_fmt_num(item.get('avgNotional'), 2))}</td>"
            f"<td>{html.escape(_fmt_num(item.get('avgExecutedFees'), 4))}</td>"
            f"<td>{html.escape(_fmt_num(item.get('avgGrossEdge'), 4))}</td>"
            f"<td{net_class}>{html.escape(_fmt_num(item.get('avgNetEdge'), 4))}</td>"
            f"<td>{html.escape(_fmt_num(item.get('commissionDragPct'), 2))}%</td>"
    ...
    body_rows = ''.join(rows) or "<tr><td colspan='12'>Пока недостаточно данных по ордерам</td></tr>"
    cache_meta = report.get('cacheMeta') or {}
    cache_generated_at = _display_local_time(cache_meta.get('generatedAt')) or '—'
    cache_delta_rows = int(cache_meta.get('deltaRows') or 0)
    cache_cached_rows = int(cache_meta.get('cachedRows') or 0)
    cache_max_rows = int(cache_meta.get('maxRows') or 0)
    cache_tracked_files = int(cache_meta.get('trackedFiles') or 0)
    return f"""
<!doctype html>
<html lang='ru'>
<head>
...
<body>
  <div class='panel'>
    ...
    <div class='muted'>Теперь страница использует tradeSummary из journal: orderId, executedQty, legs target-close/target-open, расчет gross edge, fees и net edge по тикеру. Это уже ближе к trade-result, но все еще зависит от того, что реально вернул брокер по fill в момент исполнения.</div>
    <div class='muted' style='margin-top:6px;'>cache: last={html.escape(cache_generated_at)} · delta rows={cache_delta_rows} · cached rows={cache_cached_rows}/{cache_max_rows} · files={cache_tracked_files}</div>
    ...
    <form method='post' action='/admin/effectiveness/rebuild' style='margin-top:8px; display:flex; gap:8px; align-items:center;'>
      <button type='submit'>Rebuild effectiveness cache</button>
    </form>
    <div class='cards'>
      ...
      <div class='card'><div class='label'>lifecycle trades</div><div class='value'>{int(totals.get('lifecycleCount') or 0)}</div></div>
      <div class='card'><div class='label'>sum net edge</div><div class='value'>{html.escape(_fmt_num(totals.get('sumNetEdge'), 4))}</div></div>
      <div class='card'><div class='label'>commission drag</div><div class='value'>{html.escape(_fmt_num(totals.get('commissionDragPct'), 2))}%</div></div>
    </div>
    <table>
      <thead><tr><th>ticker</th><th>broker</th><th>placed</th><th>lifecycle</th><th>errors</th><th>avg notional</th><th>avg fees</th><th>avg gross</th><th>avg net</th><th>commission drag</th><th>move to fee</th><th>lot hint</th></tr></thead>
      <tbody>{body_rows}</tbody>
    </table>
  </div>
</body>
...
def _record_webhook_decision(decision: Dict[str, Any], payload: Dict[str, Any], materialized: Dict[str, Any]) -> None:
    ...
    for dest_index, dest in enumerate(destinations):
        ...
        fee_rate = 0.0005 if broker == 'bingx' else 0.0006 if broker == 'bybit' else 0.0
        order_attempts = []
        if isinstance(req.get('orderAttempts'), list):
            order_attempts.extend(req.get('orderAttempts') or [])
        if isinstance(req.get('targetOpenAttempts'), list):
            order_attempts.extend([{**item, 'phase': item.get('phase') or 'target-open'} for item in (req.get('targetOpenAttempts') or []) if isinstance(item, dict)])
        leg_summaries = []
        for attempt in order_attempts:
            ...
            executed_qty = _to_float(attempt.get('executedQty'))
            placed_price = _to_float(attempt.get('placedPrice'))
            notional = abs(executed_qty * placed_price)
            phase = str(attempt.get('phase') or 'primary')
            final_status = str(attempt.get('finalStatus') or '')
            if notional <= 0 and executed_qty <= 0 and not final_status:
                continue
            leg = {
                ...
                'notional': notional,
                'feeRate': fee_rate,
                'estimatedFee': notional * fee_rate if notional > 0 and fee_rate > 0 else 0.0,
                'finalStatus': final_status,
                'remainingQty': _to_float(attempt.get('remainingQty')),
            }
            leg_summaries.append(leg)
        trade_legs.append({
            'broker': broker,
            'symbol': symbol,
            'venue': venue,
    ...
    if summary_needed:
        ...
        append_journal({
            ...
            'tradeSummary': {
                'destinations': trade_legs,
            },
            'details': f"payload={_short_json(payload)}" + (f" | route={_short_json(materialized, 280)}" if materialized else '') + (f" | exec={' || '.join(destination_debug)}" if destination_debug else '') + (f" | error={decision.get('error')}" if decision.get('error') else ''),
        })
...
def _enqueue_webhook_job(payload: Dict[str, Any], materialized: Dict[str, Any], default_execution: Dict[str, Any], origin: str = 'webhook') -> Dict[str, Any]:
    ...
    _append_multi_destination_journal(
        ...
        payload,
        materialized,
        lambda destination: f"jobId={job['jobId']} | payload={_short_json(payload)} | route={_short_json(destination or materialized, 280)} | queueDepth={decision['queueDepth']}",
    )
    return decision
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/admin/effectiveness/rebuild':
            if not _has_permission(current_user, 'canViewJournal'):
                return self._json(403, {'error': 'forbidden'})
            _reset_effectiveness_cache()
            return self._redirect('/effectiveness')
...
def main():
    ...
    append_journal({
        ...
        'brokers': [],
        'status': 'ok',
        'details': f'listening on http://{SERVER_HOST}:{SERVER_PORT}/ pid={os.getpid()} workers={WEBHOOK_WORKER_COUNT} metricsSync={METRICS_SYNC_INTERVAL_SECONDS}s builtAt={build.get("builtAt","")} fileCount={build.get("fileCount","")}',
